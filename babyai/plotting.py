...
def load_log(dir_):
    ...
    df = pandas.read_csv(
        os.path.join(dir_, "log.csv"), error_bad_lines=False, warn_bad_lines=True
    )
    if not len(df):
...
def plot_average_impl(
    df, regexps, y_value="return_mean", window=1, agg="mean", x_value="frames"
):
    ...
    model_groups = [
        [m for m in unique_models if re.match(regex, m)] for regex in regexps
    ]
    ...
    for regex, models in zip(regexps, model_groups):
        ...
        num_frames_per_model = [
            df_model[x_value].max() for _, df_model in df_re.groupby("model")
        ]
        median_progress = sorted(num_frames_per_model)[
            (len(num_frames_per_model) - 1) // 2
        ]
...
def plot_all_runs(
    ...
    for model, df_model in df_re.groupby("model"):
        ...
        pyplot.plot(df_model[x_axis], values, label=model, **kwargs)
...
def best_within_normal_time(
    ...
    models, num_samples = zip(
        *sorted(list(zip(models, num_samples)), key=lambda tupl: tupl[1])
    )
    ...
    for model, num in zip(models, num_samples):
        if num == max_samples:
            ...
            success_rate = (
                df_model["validation_success_rate"].rolling(window, center=True).mean()
            )
            if np.isnan(success_rate.max()) or success_rate.max() < 0.99:
                raise ValueError(
                    "{} has not solved the level yet, only at {} so far".format(
            ...
            print(
                "the model with {} samples first solved after {} epochs ({} seconds, {} frames)".format(
    ...
    for model, num in zip(models, num_samples):
        ...
        if max_within_normal_time > 0.99:
            min_samples_required = min(
                num, min_samples_required if min_samples_required else int(1e9)
            )
        if df_model[limit].max() < normal_time:
            need_more_time = True
        print(
            "{: <50} {: <5.4g}\t{: <5.4g}\t{: <5.3g}\t{:.3g}".format(
                model.split("/")[-1],
...
def estimate_sample_efficiency(df, visualize=False, figure_path=None):
    ...
    if (y > success_threshold).sum() < min_datapoints:
        raise ValueError(
            f"You have less than {min_datapoints} datapoints above the threshold.\n"
            "Consider running experiments with more examples."
        )
    if ((y > almost_threshold) & (y < success_threshold)).sum() < min_datapoints:
        raise ValueError(
            f"You have less than {min_datapoints} datapoints"
    ...
    while True:
        if (y[1:] > success_threshold).sum() >= min_datapoints and (
            (y[1:] > almost_threshold) & (y[1:] < success_threshold)
        ).sum() >= min_datapoints:
    ...
    gp = GaussianProcessRegressor(kernel=kernel, alpha=0, normalize_y=False).fit(
        x[:, None], y
    )
    ...
    for j in range(len(grid)):
        ...
        p = stats.multivariate_normal.cdf(
            np.zeros_like(mu), mu, sigma, abseps=1e-3, releps=1e-3
        )
        probs.append(p)
        total_p += p
