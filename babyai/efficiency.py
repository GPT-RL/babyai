...
def main(
    ...
    for demo_count in demo_counts:
        ...
        # Print info
        print(
            "{} demos, {} epochs of {} examples".format(
                demo_count, epochs, epoch_length
            )
        )
        ...
        cmd = (
            "{model_params} --val-episodes {val_episodes}"
            " --seed {seed} --env {env} --demos {demos}"
            " --val-interval 1 --log-interval 1 --epoch-length {epoch_length}"
            " --model {model_name} --episodes {demo_count} --epochs {epochs} --patience {epochs}".format(
                **locals()
            )
        )
        if pretrained_model:
