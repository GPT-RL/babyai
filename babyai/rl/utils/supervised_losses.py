...
class ExtraInfoCollector:
    ...
    def __init__(self, aux_info, shape, device):
        ...
        for info in self.aux_info:
            ...
            if required_heads[info] == "binary" or required_heads[info].startswith(
                "continuous"
            ):
                # we predict one number only
                self.extra_predictions[info] = torch.zeros(
                ...
                self.extra_predictions[info] = torch.zeros(
                    *shape, n_classes, device=self.device
                )
            else:
    ...
    def process(self, env_info):
        ...
        env_info = [
            {k: v for k, v in dic.items() if k in self.aux_info} for dic in env_info
        ]
        env_info = {
            k: [env_info[_][k] for _ in range(len(env_info))]
    ...
    def fill_dictionaries(self, index, env_info, extra_predictions):
        for info in self.aux_info:
            dtype = (
                torch.long
                if required_heads[info].startswith("multiclass")
                else torch.float
            )
            self.collected_info[info][index] = torch.tensor(
                env_info[info], dtype=dtype, device=self.device
            )
    ...
    def end_collection(self, exps):
        ...
        for info in self.aux_info:
            ...
            if required_heads[info] == "binary" or required_heads[info].startswith(
                ...
                extra_predictions[info] = (
                    self.extra_predictions[info].transpose(0, 1).reshape(-1)
                )
            elif type(required_heads[info]) == int:
                # T x P x k -> P x T x k -> (P * T) x k
                k = required_heads[info]  # number of classes
                extra_predictions[info] = (
                    self.extra_predictions[info].transpose(0, 1).reshape(-1, k)
                )
...
class SupervisedLossUpdater:
    ...
    def eval_subbatch(self, extra_predictions, sb):
        ...
        supervised_accuracy = torch.tensor(0.0, device=self.device)
        supervised_L2_loss = torch.tensor(0.0, device=self.device)
        supervised_prevalence = torch.tensor(0.0, device=self.device)
        ...
        for pos, info in enumerate(self.aux_info):
            ...
            if required_heads[info] == "binary":
                ...
                supervised_loss += coef * F.binary_cross_entropy_with_logits(
                    pred.reshape(-1), target
                )
                supervised_accuracy += (
                    ((pred.reshape(-1) > 0).float() == target).float().mean()
                )
